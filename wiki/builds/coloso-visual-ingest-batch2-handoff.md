...
- **ひづるめ B1 群(06/07/09/13/14)の量産完了(2026-08-25)**: ch07 41枚/6再確認・ch09 91枚/10再確認・
  ...
  **B1 群の独立レビュー(2026-08-25・条件付き承認)の修正指示 A〜C を適用済み
  ...
  修正確認レビューで指摘された ch09 ev-091 ツール名 1 字(獣→狸)の訂正適用後、修正再確認(別セッション)が
  PASS → **B1 群は 2026-08-25 承認確定**(判定正本 review/2026-08-25-hizurume-b1-fix-confirm.md・
  quality-gate hizurume-b1-theory に承認記録済み・`--phase batch` は未承認 4 family のみ FAIL で正常)。**
- **ひづるめ B2 パイロット ch15 の量産完了(2026-08-25)**: 分割 4 本・226 枚(20秒間隔+誘導 61 時刻+
  10秒スイープ 323 枚で未観測 6 件補完[エンドカード含む])・盲検読取 19 体+スイープ読取 11 体+
  第2読者 23 枚(corrected 1: p1 02m40s「方々」)・gate complete PASS・visual_ingested 付与・inbox i0826715。
  **B2 群(ch17/18/19)の量産はパイロット ch15 の独立レビュー承認まで停止中**(レビュー指示文は 2026-08-25 に武田さんへ渡し済み)。
...
- 次の一手: **B2 パイロット ch15 の独立レビュー(別セッション)の承認を受取る**(指示文は 2026-08-25 に
  武田さんへ渡し済み。承認後 quality-gate hizurume-b2-practice1 に記録→ ch17/18/19 を量産)→ B3(ch20 パイロット)へ。
...
- **marse ch08 パイロットの独立レビュー完了(2026-08-25)**: 条件付き承認。必須修正1(末尾 7:42〜7:43.37 の
  macOS 画面収録を ev-008 として収載・「観測漏れなし」虚偽3箇所の訂正・シーン検出リストへの 7:38 追加)と
  必須修正2(ev-005 の引用字をスライド実表記「賞った」へ訂正)+推奨修正3〜5 を同日適用、gate complete PASS。
  現在は **別セッションによる修正確認(必須1・2)の PASS 受取まで marse 群残りの量産は停止中**
  (確認用指示文: KB ルート `review-marse-ch08-fix-confirm-instructions.md`)。→ **2026-08-25 に別セッション
  レビュアーが PASS(量産開始可)を判定、marse 群残りの量産開始条件を満たす。**
  量産対象は実測 **13章・動画21本・合計245分**(ch09/10/11/14/21/22 単一、ch12〜20 のうち 12/15〜20 は
  2〜3本分割。ch01〜03 は raw ページ・動画とも無しで metadata-only のまま映像 ingest 対象外)。
- **ye_jji ch06 パイロットの独立レビュー完了(2026-08-25)**: **無条件承認(修正必須0件)**。
  判定正本は `review/2026-08-25-yejji-ch06-pilot-review.md`、quality-gate.json の yejji-remaining に
  承認フィールド記録済み(batch_safe: true)。→ **ye_jji 群残り14章(ch07〜23 のうち動画あり)の量産開始条件を満たす**。
- **ye_jji ch07 量産完了(2026-08-25)**: 2分割動画・108枚・corrected 1件(p1-17m40s 選択レイヤー)・
  スイープ90枚で注目0件・gate complete PASS・visual_ingested 済み・inbox i0825322。
  教訓: スイープ対象リストは手書きせず必ず機械生成して渡す(手書きで31枚の読み飛ばし→機械照合で検出・追加読取済み)。
  → **残り13章(ch08〜23 のうち動画あり)を量産**。
  パイロットで確立した ye_jji 群固有の手順: 分割動画の全パート通し観測、字幕焼き込みのため字幕変化も全行観測、
  ハングル手書きは「判読不能」処理、盲検サブエージェント応答分断時は opencode.db から本文復旧(落とし穴#10)。
- **ye_jji ch08 量産完了(2026-08-25)**: 3分割動画・216枚(p1 72/p2 68/p3 76)・corrected 4件
  (p1-04m00s 黒帯ラベル末尾は画面端切れ→切れ部分を削除/p1-17m12s Brush Size 50.6→59.6/
   p2-05m20s 選択レイヤー 암부 Copy 系に確定/p3-15m40s R値73→173)+p3-19m40s 見出し「高密度な彩色の間違いノート」訂正・
  スイープ167枚で注目0件+作業ファイル名「색감 실습.jpg」「색감 실습2.jpg」を原寸クロップで確定し表内統一・
  gate complete PASS×2・visual_ingested 済み・inbox i0825651。PC 再起動で1回中断したが staging 退避により無傷再開。
- **ye_jji ch09 量産完了(2026-08-25)**: 2分割動画・156枚(p1 70/p2 86)・corrected 2件
  (p2-07m20s 選択レイヤー Layer16 確定/p2-19m23s 選択レイヤー「Layer 13 Copy 2」相当行に確定・同一レイヤー16行の表記統一)・
  スイープ121枚で注目0件・gate complete PASS×2・visual_ingested 済み・inbox i08257f9。
  教訓: ドキュメント名「9장 명도 낮추기」等ハングル UI 名は読み揺れが大きい→ヘッダ凡例に注記して個別断定を避ける。
  → **残り11章(ch10〜ch20 のうち動画あり)を量産**(下記セッション開始指示文 ye_jji 継続分を使用)。
