...
## 2026-09-09 ユーザー訂正（前の追補より優先）

- 前の作業でヘレンの参考画像に通常衣装を選んだのは誤り。ユーザーが指定しているのは原作再現プロジェクトで抽出済みのドレス。直接確認した正しい参照ファイルは `/Volumes/SSD_M.2_Realtek RTL9210 NVME Media_/01_イラスト/07_3D資料/gf2-helen-starlit-waltz/06_repro-v51/blends/helen-h0157-repro.blend`、SHA-256 `04ef8b79b3fa5b64b9d7e3496a9adc184f10c07d9ee9758caebd289ddbb6d7f5`。SSR0101のドレスとFlatを含む実オブジェクト、保存フレーム300、既存カメラを確認した。背景撮影のみで元Blendは不変。
- 比較の目的は、胸の大きさ・位置と、衣装が胸を支える位置、紐の引かれる方向・接触・浮きの関係を3体の実データから読むこと。ユーザーの言葉：「つくりの構造を真似る話じゃなくて、比較して何がわかるか」「貧乳に合わせた原型を巨乳に無理やり合わせるという作業」。サブリナを部品の分割比較だけ、ヘレンを体の寸法だけに限定しない。
- 過去のヘレン部品の同定失敗を、ドレスを比較対象から除外する根拠として採用しない。3体の対応箇所と姿勢を整えた比較はまだ未完了。
- 先に提示した「両端固定・短縮・特定区間だけ外へ戻す」案は撤回。比較の前に接続点固定を条件にしない。HTMLの該当節と概念図を外し、正しいドレス画像と比較目的へ訂正した。旧画像・訂正前HTMLを残し、元Blendは無変更。
- 再開点は、正しいドレスを含めて体と衣装の対応箇所を直接取り直し、3体比較から載せ替え方を判断すること。今回のHTML訂正をこの比較の完了として扱わない。

## 2026-09-09 再開時の追補（この節を先に読む）

- 指定タスク `01a07f42-79f2-73d3-ae92-279054eda733` は、3体比較HTMLを書いた直後に利用上限で停止。水着の次候補の実装開始地点ではなかった。今回、当該HTMLの内容・根拠・表示を確認して修正した。
- 説明ページ： [3体のお手本があるのに、肩ひもで詰まる理由](../_attachments/helen-swimsuit-status/20260908-why-three-references-do-not-fix-strap.html)。構造・用語・タイムラインの確認に加え、背景ブラウザで1440pxと390pxの横幅、画像5枚の読込、目次の参照先を確認。参考画像の並びと概念図は描画画像を目視した。ユーザー画面での確認ではない。
- 現行Blendの実ファイルSHA-256は `d6ee7ed4f0c0043257cf7414f5ff3646f42fa1fdf91a5e94bb70deb855aefebf`。下の9月7日記録の `b677e020…` を現行値として使わない。今回Blendは変更していない。
- 9月8日の別名候補は不採用。`output/gf2-helen-swimsuit/compare-straps-final-20260908.json` を直接確認：右の道のりは約2.06mm短縮したが落ち込みは不合格のまま、両肩の測定点のめり込み割合は増加。「しわが減った」という見た目の断言はHTMLから除去した。
- 参考画像はドゥルシーヌヴイとヘレンの2枚を重複のない表示選択で背景撮影し、サブリナは重複なしの既存画像を使用。表示部品と元Blendの不変性は `wiki/_attachments/helen-swimsuit-status/20260909-reference-render-evidence.json`。ドゥルシーヌヴイの白抜け・一部欠けは残るため、全身や材質の完全再現画像ではない。正確な接触距離は写真でなく座標比較を根拠にする。
- 使わなかったもの：下書きの参考画像2枚を差し替え。衣装・髪の重なりが減る。ドゥルシーヌヴイの材質未設定の174点の部品 `c_DusevnyjSSR0101_slg_P3_body1_lod0`、武器・効果・別衣装を撮影から省略した。174点の部品単独の見た目への影響は未確認。旧画像と元Blendは残るので、画像の参照を元に戻せば復元できる。3D資料本体の形状と表示設定は変更していない。
- 残る作業：肩ひもの張りと接触を同時に満たす形状修正。局所補正の方法は未決。帯・小物の保留を維持。今回の再開は説明HTMLの仕上げまでで、水着完成・候補採用・新しい修正方法の承認を記録していない。別タスク固有のスキル修繕・待機状態は今回へ引き継いでいない。
