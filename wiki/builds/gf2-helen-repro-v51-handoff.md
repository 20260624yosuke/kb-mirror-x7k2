...
# 引き継ぎ資料 — HELEN-REPRO v5.1（2026-08-20 作成・**2026-08-27 更新**）

> [!warning] 2026-09-01 追記 — 現在位置の正本は移りました
> HELEN-REPRO v5.1 の **現在位置・再開入口の正本** は
> `wiki/builds/gf2-helen-repro-v51-current.md` です。このページは **引き継ぎの全文（履歴）** として残しています。
> 本文中の「衝突したらこちらが新しい」は作成当時（2026-08-27 以前）の主張です。
...
> [!warning] 2026-08-27 の現在位置 — f166棚卸しで技術的停止
> 2026-08-26夜、ユーザー承認「棚卸しのみ先に」の範囲で f166 初回全量棚卸しを実行した。結果JSONは存在するが、`scripts/f166_code_inventory.py` が22:49頃に最終正規化修正された後の全量再実行・整合確認は未実施。f166は最終完了・監査合格ではなく技術的停止。強制DLと成果物blend変更は未承認。
...
- **f166コード資産棚卸し**: 初回全量走査は実行済みだが、最終正規化修正後の再走査・整合確認前に技術的停止（詳細は下の2026-08-27追記）。
...
> [!warning] 2026-08-26 武田さんの指示（監査規律）
> - **「ローカルは全て見た」は常に誤り前提**。探索済み範囲の列挙＋未走査領域の明示を必須とし、
>   「全て見た」という言い方は禁止（弱い形のみ）。
> - **「ローカルに無いがゲームから取得できる」等の能力主張は、実施記録なしには書けない**
>   （f72の能力主張枠を取得能力側にも適用）。
> - 判断の基準は**成果物のビルドクオリティ**。解析は手段であり、報告はS6〜S9ゲートと
>   残差リストからの逆算で並べる。
...
### 2026-08-27 同期 — f166コード資産棚卸し（初回全量実行・技術的停止）

親セッションは `ses_fc4093be6ffeK5wnAZYgDrrUVf`。実作業の子セッションは `ses_fc212669affe1ryOajM6Xaul5g`、再試行は `ses_fc1a4b8feffey6oPHSir1B19I1`。ユーザーが承認した範囲は「棚卸しのみ先に」。強制DL、監査強化、成果物blend変更は承認されていない。

- 「ローカルコードを全て抽出した」と言える網羅性根拠が無いことが判明し、未調査候補として vertex MSL、compute kernel、平文PE DLL、Lua、MonoScriptとDLLの対応、シェーダバリアント表が挙がった。
- 初回全量結果 `logs/f166-code-inventory.json`（`generated_at` 2026-08-26T22:33:35.013919+09:00、`finished_at` 2026-08-26T22:42:48.421543+09:00）と `logs/f166-code-inventory-status.json`（22:42:48頃）が存在する。`scripts/f166_code_inventory.py` は22:49頃にも変更された。
- MSLは **18,568 bundle走査、ユニーク文書7,726種、既存抽出との未一致7,424種**。未一致内訳は fragment 5,553種、vertex 1,855種、kernel 63種。結果JSONの `pipeline_valid` は true だが、これは最終修正版による再走査を意味しない。
- 平文PE DLLは **33本、メタデータ読取可能33本**。Luaは **1,031本、Lua 5.3ヘッダ1,030本、主チャンク定数抽出成功1,030本**。MonoScriptは **14,534件、distinct assemblies 62**。DLLとの部分突合判定は `possible_partial`。
- `scripts/f166_code_inventory.py` の最後の変更は、既存post MSL末尾のメタデータ混入をNUL位置で正規化する修正。**この最終修正版による全量再実行、整合確認、Wiki同期前の完了根拠は無い**。
- 2回目の子セッション `ses_fc1a4b8feffey6oPHSir1B19I1` はモデル非対応エラーで終了しており、完成根拠にはしない。強制DLは未承認のまま。

**現在位置**: 初回全量棚卸しは実行済みで大規模な未抽出コード群を検出したが、最終正規化修正後の再走査・整合確認前に技術的停止。f166を「最終完了」「監査合格」とは記録しない。
...
## 2026-09-05 正本時刻割れの整合（opencode作業分・事実のみ）

- `run-state.json` の history を113→117件へ追記し、`last_updated` を8-26→9-05へ更新した。追記は作業事実のみで、判断文の変更なし。
- `quality-gate.json` は規則により不接触（8-25のまま）。S6/S8/G10 の値は変化なし。
- 9-05作業：段階0凍結／画像認識の当座禁止／門7本の候補限定封鎖（範囲は当該対話の実行者のみ、門自体は無改変）／読側・書側符号の適用順特定／候補6本（前段倍率3・主光3）／指の左手反転誤りの数値特定／f171三工程説明門の新設と再現試験6件合格。
- 本体blend無変更（`04ef8b79b3fa5b64`）。次は左手反転式の修正候補（本体反映は別承認待ち）。
