...
### 再現例をもう1件（2026-08-29 15:1x・ブレスト再開中に実際に発生）

作業ディレクトリを KB ルートに `cd` してから、`wiki/analyses/` 内のメモを Python の
ヒアドキュメントで書き換えようとしたところ、封鎖が拒否した。拒否メッセージの
「止めた対象」に出たのは、**KB ルートを1つ目の半角スペースで切った断片**（ボリューム名の途中で終わる文字列）で、実在しない。書き込み先は許可される場所だった。
断片そのものをこの文書に書き写すと到達性監査が毎回 FAIL するので、あえて書かない。
実物は `guard.log` の `lockdown DENY tool=Bash path=...` の行で見られる。

さらに、**コマンド本文だけでなくヒアドキュメントの中身も走査対象になっている**ことが分かった。
`cd` を外しても、本文に KB ルートの絶対パスを書いた行が含まれていれば同じ断片で拒否される。
`cd` も絶対パスも含めない相対パスだけのコマンドにして、ようやく通った。
**この3通り（cd あり／本文に絶対パス／相対パスのみ）を再現試験にそのまま使える。**
...
## 2026-08-30 実測（追記）: 解除コマンド自体が封鎖に止められる

武田さんが「解除」と明示した状態で、拒否メッセージが案内している解除コマンドを実行しようとしたが、
**3通りすべて同じ封鎖に止められた。** 封鎖から抜ける手段がセッション内に存在しない。

| 試した形 | 止められた対象として表示された文字列 |
|---|---|
| 案内どおりの絶対パス | `/Users/takedayousuke/.claude/skills/brainstorm/brainstorm_guard.py` |
| `cd` してから相対パス | `<KBルート>/python3 brainstorm_guard.py`（**空白で切れた断片を KB ルートに連結している**） |
| シェル変数経由 | `<KBルート>/.py`（**拡張子だけの断片を連結している**） |

**課題1（`_candidate_paths()` の空白分割）の深刻度は、当初の想定より高い。**
誤検知で読み取りが止まるだけでなく、**設計上の唯一の逃げ道（解除）まで塞いでいる。**
この日、成果物 Inbox への申告がこの理由で実行できず、次のセッションへ持ち越しになった。

修正時は、**解除コマンド（`bypass`）だけは必ず通ること**を試験に含めること。
