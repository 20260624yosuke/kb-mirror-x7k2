...
revision: 3
parent_plan: gf2-helen-h0157-current-app-reextract-workflow-plan-v3-20260914
# `implementation_started` は P0〜R3 の実装に限定した boolean。Q0・Q0-CR・差分再審査フローは本番反映済み。
implementation_started: true
implementation_gate: r1-remediation-stop
blocking_evidence: R1-OBJECT-COORDINATE-SPACE-SCHEMA-LIMITATION-01
blocking_evidence_superseded: EA_KB_SNAPSHOT_STALE (2026-09-10; Q0昇格で解消)
current_state_reviewed: 2026-09-14
historical_state_20260911:
  implementation_started: false  # historical 2026-09-11, non-authoritative
  implementation_gate: blocked  # historical 2026-09-11, non-authoritative
  blocking_evidence: EA_P0_NOT_AUTHORIZED  # historical 2026-09-11, non-authoritative
  current_state_reviewed: 2026-09-11  # historical 2026-09-11, non-authoritative
  execution_status: q0-complete-p0-not-authorized  # historical 2026-09-11, non-authoritative
supersedes:
...
execution_status: q0-p0-r0-pass-r1-remediation-stop-r2-not-authorized
...
object-inventory.jsonl:
  required: [object_id, container_id, coordinate_space, object_type, source_offset,
             source_length, disposition, parser_id]
...
### 8.1 objectとcoordinate spaceの束縛

- object行の `source_offset` / `source_length` は、その行の `coordinate_space` に対する座標として定義する。
- object行の `coordinate_space` は、同一 `container_id` の range-inventory に存在する値と一致しなければならない。
- `extracted` / `non-code` かつ `length > 0` の object は、同一 container・同一 space の
  `extracted` / `non-code` / `padding` ranges によって完全に被覆されなければならない。
  `coverage-open` / `failed` の object には適用しない（明示的に未解析のため）。
- verifier は object row が宣言した `coordinate_space` だけを検証する。
  別spaceでの偶然の被覆を代用にしない。
- container行の `coordinate_space` は、その container 自身のbytesの空間
  （primary/raw source space）を指す。child の object/range は derived spaces を持てる。
  複数spaceを持つ container があってもよい。
- range行の `parent_id` は `container_id` のまま変更しない。
  nested object 同士は重なり得るため、range を object へ所有付けしない。
  tiling と被覆は (container, space) 単位で検査する。
- object行の `coordinate_space` は builder が任意に選ぶ値ではない。
  固定parserがその object を列挙した親source/child space から決定される。
- verifier は live input と sealed parser/runtime から object を独立再列挙し、
  少なくとも `(container_id, coordinate_space, object_type, source_offset,
  source_length, disposition, parser_id)` の multiset を成果物と比較する。
  宣言との一致だけでなく、parser由来の再構成を必須とする。
- UnityFS では archive-file/resource-stream/class-* の space を、
  BundleFile.files の親entry名 / SerializedFile親 / resource stream親から
  再構成する。
- 同一 class/start/size または同一 size の双子が複数spaceに存在しても、
  object row の宣言だけを tie-break 根拠にしない。parser由来の親space
  identity を根拠にし、存在する space ごとに 1 row ずつ数える
  （重複内容の各 instance が実在単位であるため）。
- verifier が親space identity を独立再構成できない unit は、
  `extracted` / `non-code` として PASS させず、`coverage-open` または
  `failed` に残す。
...
- P0許可の状態: authorization成立。P0 canonical registry は独立review PASS
  （旧 `EA_P0_NOT_AUTHORIZED` は解消済み。2026-09-11 値は
