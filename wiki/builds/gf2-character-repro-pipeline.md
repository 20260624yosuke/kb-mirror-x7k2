...
### 原則として守ること（2026-08-25・武田さん判定）

- **「原作データどおり」は原作の見た目と矛盾する場合は欠陥**。blend⇔抽出データの
  一致（f129系の循環検証）では抽出段階の誤りを検出できない。指の曲がり・顔の筋・
  生え際のように原作に無い見た目異常が出ているなら、データ照合が一致していても
  抽出/適応の誤りを疑う。正解は原作フレームとの直接比較のみ。
- **見た目比較門を監査に入れる**: 構造照合（S1）・データ照合（S2/S3b）だけでは
  「ザル」。顔クロップ比較・指角度・髪被覆・スペキュラ有無など、原作フレームと
  の直接比較門を必ず設ける（HelenでS6〜S9として新設予定・詳細は
  [[gf2-helen-repro-v51-handoff]] #73）。
