...
>   `tools/render_set_check.py`（D5a 変種の重複 / D5b 役割の重複）を新設し、
>   `tools/deliverable_path_guard.py` の `check()` から **guard-card（承認カードの直前）と
>   guard-stop（会話の終わり）の両方**で判定する。
>   **D5 単体 7/7 ／ パスの関所と合わせて 14/14・配線 5/5。KB のレンダー 15 件中 不合格 1 件。**
>   **ドゥルシーヌヴイの既存 repro レンダーも D5 で落ちる**（未対応）。
> - **【2026-09-07】紐は「厚み」で同定する。「主軸の細長さ」では曲がった紐を取り逃す。**
>   基準: 厚み（いちばん薄い方向の広がり）≤ 12mm かつ 首まわりの方位の広がり ≥ 90 度。
>   ドレス本体（124.7mm）・カップ（68.9mm）と明確に分かれる。
>   **ヘレンのドレスには首を回る紐が 3 種類ある**（92頂点/5.94mm・70頂点/8.60mm・46頂点/5.44mm）。
> - **【2026-09-07】お手本の Blend の実パス**（**repro の 3 本は変種が全部可視のまま。
>   開いたら同じ混在が見えるので、まず変種を 1 つに絞ること**）:
>   `…/gf2-char-extract/blends/Dusevnyj-DusevnyjSSR0101-repro.blend`（44MB）／
>   `…/blends/Sabrina-SabrinaSSR0101-repro.blend`（13MB）／
>   `…/blends/Helen-HelenSSR01-repro.blend`（62MB）
