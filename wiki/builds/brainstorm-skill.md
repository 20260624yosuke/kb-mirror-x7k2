...
| `guard-stop` | Stop（スキル起動中） | 承認カード無しで会話を閉じようとしたら止める（2026-09-07 修正。**承認が終端になるのは直後の最終報告1回だけ**。以前は一度カードに答えると以後どのターンも素通りしていた） |
...
## 2026-08-31 の追加（発言の記録とスキル起動を機械で確かめる）

### 穴C・D 発言がメモに残っていなかった

実測: この会話で武田さんの実質的な発言 **12件中6件（50%）がメモに残っていなかった**。
原因は `guard-stop` が「承認カードがあるか」しか検査しておらず、**メモへ書いたかを見ていなかった**こと。
「毎ターン書き足す」は SKILL.md の文言ルールのままだった。

**実装**: `guard-stop` に**メモ反映の検査**を追加（カード検査より前）。直近の発言から12文字以上の
断片を取り、生きているメモに半分以上残っていなければ**会話を閉じさせない**。
30文字未満と短い承認語は検査しない。

### 穴E スキルを名指しされても起動しなかった

実測: `/html` はカード回答に **10回**現れたが、`<command-name>` として認識されたのは
**`/brainstorm` の2回だけ**。`Skill` の呼び出しは会話で **1回**のみ。
**カード回答の中の `/名前` は、ハーネスから見るとただの文字列**でコマンドにならない。

**実装**: `guard-stop` に**スキル起動の検査**を追加。直近の発言の `/名前` のうち、
**2か所のスキル置き場**（`~/.claude/skills/` と `<KBルート>/.claude/skills/`）のどちらかに実在し、
その後 `Skill` 呼び出しが無いものがあれば**会話を閉じさせない**。
`brainstorm` 自身と、スキルでない語（`/tmp` 等）は対象外。

> スキルが2か所にあることは実測で確認した。`html` は KB 側にあり、ホーム側だけ見る実装では
> 取りこぼす（最初の実装で実際に取りこぼし、試験で判明した）。

### 副作用を1件出して直した

`active` を未読検査へ加えた直後、**メモを書く操作（`ready` 昇格を含む）まで拒否**され、
既存の自己試験の発火点①が偽陽性で落ちた。書き込み先が `wiki/analyses/` 配下のときは
未読検査を掛けない形にして解消。

### 試験結果

3つの穴で**合計15件**の試験を実行し、**すべて合格**。
既存の `audit-handoff --selftest` も第1層〜第3層すべて PASS。
報告: `wiki/_attachments/project-hub-index/20260831-brainstorm-three-holes-fixed.html`

**未確認**: 実機での発火。次に `/brainstorm` を閉じるとき `guard.log` に
