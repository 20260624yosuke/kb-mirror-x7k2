...
この候補を現行工程として採用するなら、最終的な見た目は武田さんの目でしか判定できない。
その場合に返してもらう候補項目は2点だった。
顔が白く飛んでいないか。指が原作の形に見えるか。

ただし、この候補と段取りはMuseの作業に由来する。現在の詰まりへ接続することを確認するまで、
F12確認を「待っている1手」や「未着手作業」として扱わない。採用する場合はsha256も測り直す。
...
「未開封はほぼ尽きた」。3つとも撤回した。
最後のものは、撤回の直後に、前任者が見落としていた詳細記録を照合したことで覆った。
`f198-gff-unopened-accounting.json` には248MB目録56,846件の名前復号と、探していた2本の名前が
すでに記録されていた。「誰も走査していなかった」という前提自体が誤りだった。
...
武田さんが2026-09-09に許可した作業を、未着手と一括りにしない。
...
1. 保存領域2台（`/Volumes/HDD_バックアップ` と `/Volumes/HDD_バックアップ_macbookpro`）の
   ...
   検索そのものが走っていなかった。
   `timeout` を外した列挙では、両ボリュームから `*.bundle` が **1,458 件**返った。
