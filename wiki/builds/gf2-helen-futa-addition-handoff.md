...
- **③移植計画書 — v2 改訂済み・承認待ち（2026-08-23 10:40頃）**
  正本: プロジェクト側 `07_futa-helen/reports/TRANSPLANT-PLAN-2026-08-23.md`。
  Phase A（スケール候補レンダ→武田さん選定）→ Phase B（本移植＋造形ループ）→ Phase C（骨＋物理揺れ）。
  **v2**: 武田さんの指示でサブエージェント独立レビューを実施（総合判定「要修正」）。主要指摘3件を
  すべて反映 — リンク読み込み→アペンド化（リンクは親子付け・材質置換・ウェイト塗り不可のため）、
  Rest Position 前提の明記、完成条件2・3の検証成果物定義。軽微指摘は採用（Rigify文言削除・
  親骨事前確認・原点正規化・UV注意書き・cloth sim先行）と後回し（GFOutline）に分類済み。
  承認が出たら Phase A から実施する。実装開始時に `quality-gate.json` をプロジェクト直下へ作成。
