...
supersedes:
  ...
  - gf2-helen-h0157-current-app-reextract-workflow-plan-v2-20260910
first_task_contract: gf2-helen-h0157-current-app-reextract-task-contract-v3-20260914
...
- **2026-09-11 当時の観測（superseded）**: Q0・Q0-CR・差分再審査フローが本番反映され、同じ検査は
  exit 0 で `品質ゲート: PASS (plan)` を返す。正本の現物SHAは
  `e66c16d684b2ea23e49dfb850a1ec57e0e9b427967451a6f6b336bf0e3fbd703`。
- **2026-09-11当時の観測（historical）**: 当時の blocker は `EA_P0_NOT_AUTHORIZED` で、
  registry の `p0_authorization.authorized` が false のため、
...
| R1 coverage-inventory | 廉価 | container/object/range目録（object行はcoordinate_space宣言付き） | 4分母が一致し、gap/overlapがなく、space-qualified object multisetを独立再構成して一致 |
...
Q0からR3の機械仕様は
[[gf2-helen-h0157-current-app-reextract-task-contract-v3-20260914]]を正本とする。
...
object_denominator:
  ...
  binding: each object row declares coordinate_space; offsets are coordinates in that space;
    verifier checks the declared space only (never substitutes coincidental coverage elsewhere)
range_denominator:
...
各spaceごとに別式を持つ。
container行のcoordinate_spaceはprimary/raw source spaceを指し、childのderived spacesは
range行のspaceで区別する。object行は自らのcoordinate_spaceを宣言し、verifierはそのspaceだけを検査する。
...
- P0許可の状態: authorization成立、P0 canonical registry は独立review PASS
  （旧値は `historical_state_20260911` に保存）。
