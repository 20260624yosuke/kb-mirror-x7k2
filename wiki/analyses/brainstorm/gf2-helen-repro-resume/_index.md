...
### 2026-09-03 表示の混乱と特定（詳細は子メモへ分離）

UI表示の混乱から表示経路の特定までの往復は、親の文脈汚染を避けるため
`sessions/20260903-vscode-visibility-detour.md` へ移した。結論は3点。
①従来の本文経路はこの環境で非表示で作業表示と選択肢だけが見えていた
（実セッション集計で確定：assistant 85件中 text 3件）。
②以後は可視の会話経路で話し、確認カードは明示承認が必要なときだけ出す。
③再発防止の機械監査を `.opencode` 配下に置いた（可視性監査スクリプトと関所の可視性規則）。
共有機構・codex・wiki正本は不変。図は名前指定で発見済み。受け入れ可否は未決のまま。
...
### 2026-09-03 表示経路の特定・会話経路へ切り替え

> これが見えるかってこと？見えるよ。

会話文の表示経路を特定した。従来の本文経路はこの環境で非表示で、作業表示と選択肢だけが見えていた。
証拠は貼り付け2件（編集差分・検査コマンドのみ可視）、名前指定での図の発見成立、可視経路での文章への内容理解の返答。
以後は可視の会話経路で話し、確認カードは実装承認など明示承認が必要なときだけ出す。
...
> ここまでの会話で俺の苛立ちと指示は明確だと思うんだけど、どうなってんの？説明して。/html

名前だけの指定で図の発見が成立した。以後の場所案内は名前指定を正とする。
苛立ちの指摘を受け、何が起きたかの説明HTMLを新規作成した。対象は表示の混乱の経緯と今の状態。
制約はwiki整合性の維持・この環境限定・codex等への影響なし。図本体の改訂は含まない。
...
どちらも不採用。原因の系統は残るが修正方向は未確定。次の候補は原作比較
（原作でも伸びていれば直す対象ではない）。正規と台帳は指紋で無変更を確認。
記録は試験記録・結果HTML。

### 2026-09-04 実探索の完了（原因の系統を特定）

承認ずみの実探索を実行した。e06再現は胸・スカート・顔が完全一致（手袋のみ46→53の
境界ゆらぎ、対象外）。骨ペア調査で「違う骨」説を棄却し、重み配合の距離で機械的因果を確定
（伸び側は正常側の4〜10倍、主骨が同じ辺だけでも差は残る）。修正試験は変更のため別承認。
3D本体と正規台帳は指紋で無変更を確認。記録は探索記録・結果HTML。
...
確定は本人の承認で行うため実行せず、棚卸しHTMLを残した。

### 2026-09-03 A案区切りと独立照合の完了

「タスクを進めます」を合図に自律実行した。A案は推定混じりの適用を扱わず、
周囲光の確定で区切った。第1契約は別セッションの独立照合で7項目すべて一致し受領可
（候補と台帳の実測SHAも一致）。作成側の自己受領はしていない。残るは契約の確定。
記録は区切り宣言・照合受領書・結果HTML。
...
基準置換えは「確定」、A案別計画は「承認して進める」、第1契約は「伸びた辺で進める」。
実行は自律的に進め、第1契約の提示時点で再び承認待ちに戻る。
...
- 2026-09-03 同型の欠落を復旧（「図受け入れから進める方針を承認」の見出し行だけが消え本文が残存。見出しを戻した。内容の削除なし。以後は見出し行だけの置換をしない）。
...
41. U3実探索の記録（2026-09-04・系統を特定・修正は別承認）: /Volumes/SSD_M.2_Realtek RTL9210 NVME Media_/05_claude/claude_llm_wiki/LLM Knowledge Base _01/wiki/analyses/brainstorm/gf2-helen-repro-resume/sessions/20260903-u3-exploration-record.md
