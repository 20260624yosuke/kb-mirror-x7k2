...
数値書換えは行わない。独立再計算で真の差0.00度と確定した
（`06_repro-v51/logs/t1-finger-diff-reverify.json`）。
残る見た目の問いは方針判断が必要。方向試験は別途承認済みと記録にある。

- 対象（読むだけ）
  - `06_repro-v51/ledger/finger-axis-decision.json`
  - `06_repro-v51/logs/t1-finger-diff-reverify.json`
- 手順
  1. この件の数値作業は終了した。再計算はしない。
  2. 残る見た目の問いは武田さんが決める。
- 完了条件は真の差0.00度の記録の維持と本体の無変更。
