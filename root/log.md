...
- 决定内容: パイロットの独立レビューを新規コンテキストのサブエージェント(実行セッションの記憶なし・動画から自前再抽出で突合)が実施し、その PASS をもって同群の量産開始条件とする。ユーザーの事前承認は不要、群完了後に成果物 Inbox で事後確認。`quality-gate.json` の stop_conditions と新設の approval_delegation に記録。
